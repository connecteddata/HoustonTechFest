        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        if (null == UserManager.GetClaims(user.Id).FirstOrDefault(c => c.Type == "urn:linkedin:accesstoken"))
                        {

                            if (null != (claim = info.ExternalIdentity.Claims.FirstOrDefault(c => c.Type == "urn:linkedin:accesstoken")))
                                await UserManager.AddClaimAsync(user.Id, claim);
                        }

                        if (null == UserManager.GetClaims(user.Id).FirstOrDefault(c => c.Type == "urn:connectedData:LinkedInUserId"))
                        {
                            if (null != (claim = info.ExternalIdentity.Claims.FirstOrDefault(c => c.Type == "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier")))
                            {
                                //update claim for specific application access
                                var transformedClaim = new Claim("urn:connectedData:LinkedInUserId", claim.Value);
                                await UserManager.AddClaimAsync(user.Id, transformedClaim);
                            }
                        }
                    }
